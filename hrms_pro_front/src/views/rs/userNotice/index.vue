...
<div class="search">
        ...
        <Row v-show="openSearch" @keydown.enter.native="handleSearch">
            <Form ref="searchForm" :model="searchForm" inline :label-width="0">
                <Form-item ref="searchForm" :model="searchForm" inline :label-width="0" style="display:flex;">
                    ...
                    <Form-item label="" prop="sendTime">
                        <DatePicker type="date" v-model="searchForm.sendTime" placeholder="请选择" clearable style="width: 200px"></DatePicker>
                    </Form-item>
                    <Form-item style="margin-left:10px;" class="br">
                        ...
                        <Button @click="noModal=true" type="info" size="small" icon="md-add" ghost>添加公告</Button>
    ...
    <Modal v-model="noModal" title="新增新闻公告" on-text="确认发布公告" @on-ok="addUserNoticeFx">
        <Row>
            <Input v-model="addNoTitle" placeholder="请输入公告标题..." style="width: 100%" />
        </Row>
        <Divider dashed />
        <Row>
            <Input v-model="addNoCon" type="textarea" :rows="8" maxlength="500" show-word-limit placeholder="请输入公告内容..." style="width: 100%" />
        </Row>
    </Modal>
</div>
</template>
...
export default {
    ...
    methods: {
        ...
        addUserNoticeFx() {
            var that = this;
            addUserNotice({
                title: that.addNoTitle,
                content: that.addNoCon
            }).then(res => {
                if (res.success) {
                    that.getDataList();
        ...
        getDataList() {
            this.loading = true;
            getUserNoticeList(this.searchForm).then(res => {
                this.loading = false;
                if (res.success) {
