    ...
    <div class="sidebar-menu-con menu-bar" :style="{width: shrink ? '60px' : '220px', overflow: shrink ? 'visible' : 'auto'}">
      <shrinkable-menu :shrink="shrink" @on-change="handleSubmenuChange" :theme="menuTheme" :before-push="beforePush" :open-names="openedSubmenuArr" :menu-list="menuList">
        <div slot="top" class="logo-con">
          <div v-show="!shrink" class="logo-main">
            <Icon type="ios-people" size="40" style="color: #ff6f00; margin-right: 10px;"/>
            <span style="color: #fff; font-size: 18px; font-weight: bold;">HRMS Pro</span>
          </div>
          <div v-show="shrink" class="logo-mini">
            <Icon type="ios-people" size="32" style="color: #ff6f00;"/>
          </div>
        </div>
      </shrinkable-menu>
    </div>
    <div class="main-header-con" :style="{paddingLeft: shrink?'60px':'220px'}">
      <div class="main-header">
        <div class="navicon-con">
          <Button :style="{transform: 'rotateZ(' + (this.shrink ? '-90' : '0') + 'deg)', height: '48px', color: '#fff'}" type="text" @click="toggleClick">
            ...
            <Row type="flex" justify="end" align="middle" class="user-dropdown-innercon">
              <Dropdown transfer trigger="hover" @on-click="handleClickUserDropdown">
                <a style="display: flex; align-items: center; color: #fff;">
                  <Avatar :src="avatarPath" style="background: #fff; margin-right: 10px;"></Avatar>
                  <span class="main-user-name" style="color: #fff;">{{ username }}</span>
                  <Icon type="md-arrow-dropdown" style="color: #fff;" />
                </a>
                <DropdownMenu slot="list">
                  <DropdownItem name="changePass">Change Password</DropdownItem>
                  <DropdownItem name="ownSpace">My Portal</DropdownItem>
