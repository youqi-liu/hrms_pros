...
import cn.lyp.basics.exception.LypAuthException;
...
public class AuthenticationFailHandler extends SimpleUrlAuthenticationFailureHandler {
    ...
    public boolean recordLoginTime(String username) {
        ...
        int loginFailTime = 0;
        if(!LypNullUtils.isNull(loginFailTimeStr)){
    ...
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException exception) {
        if (exception instanceof BadCredentialsException || exception instanceof UsernameNotFoundException) {
            ...
            int userFailTimes = 0;
            if(!LypNullUtils.isNull(failTimesStr)){
        ...
        } else if (exception instanceof LypAuthException){
            ResponseUtil.out(response, ResponseUtil.resultMap(RESPONSE_FAIL_FLAG,RESPONSE_FAIL_CODE,((LypAuthException) exception).getMsg()));
